...
void find_line_left(Table* table, int* index, int index_2, int* hight, int* index_col, int* line) {
	int index_1 = *index;
	int line_1 = 0;
	...
	while (table->array[index_1][index_2] != table->full) {
		...
		line_1 = find_line_left_1(table, index_1, index_2);
...
int find_way_left(Table* table, int* index, int* hight, int* index_col) {
	...
	while (table->array[*index][index_1] != table->full) {
		...
		ptr = find_way_1_1(table, *index, index_1, hight_1);
...
void fill_line_left_1(Table* table, int* index, int* entry, int* hight, int* index_col, int* count) {
	check_way_left(table, index, entry, hight, index_col, count);
...
void check_hight(Table* table, int* index, int index_1, int* hight, int* index_col) {
	...
	find_hight(table, index, index_1, &hight_1, &index_col_1);
	if (hight_1 == table->line) {
		*hight = hight_1;
		*index_col = index_col_1;
	}
}
...
void fill_line_left(Table* table, int* index, int* entry, int* exit, int* count) {
	...
	while (table->array[*index][index_1] != table->full && index_1 < *exit) {
		...
		if (table->line - hight < *exit - index_1 && hight != table->line) {
			find_line_left(table, index, index_1, &hight, &index_col, &line);
		}
		index_1 ++;
		if (index_1 == *exit) {
	...
	fill_line_left_1(table, index, entry, &hight, &index_col, count);
}
